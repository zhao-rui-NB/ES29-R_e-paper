...
typedef struct
{
     ...
     * |        |          |Note: This bit field is for Master mode only.
     ...
     * |        |          |If the valid data count of the receive FIFO buffer is greater than the RXTH setting, the RXTHIF bit will be set to 1, else the RXTHIF bit will be cleared to 0.
     ...
     * |        |          |b. For SPI Master mode, SPIx_CTL[0] = 1 and TXEMPTY = 1 but the current transaction is not finished yet.
     ...
     * |        |          |1 = The valid data count within the receive FIFO buffer is greater than the setting value of RXTH.
     ...
     * |        |          |0 = The valid data count within the transmit FIFO buffer is greater than the setting value of TXTH.
     ...
     * |        |          |If this bit is set to 1, when right channel data sign bit changes or next shift data bits are all 0 then RZCIF flag in SPIx_I2SSTS register is set to 1
     ...
     * |        |          |0 = SPI/I2S control logic Disabled.
     * |        |          |1 = SPI/I2S control logic Enabled.
     * |        |          |Note: The SPI peripheral clock is asynchronous with the system clock
