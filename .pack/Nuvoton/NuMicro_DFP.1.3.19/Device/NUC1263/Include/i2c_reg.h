...
typedef struct
{
     ...
     * |        |          |Note: When software sets 10u2019h000, the address cannot be used.
     * @var I2C_T::DAT
     ...
     * |        |          |The TIDLE is used to calculate the time-out of clock low in bus active and the idle period in bus Idle
     * |        |          |This bit is used to define which condition is enabled.
     * |        |          |0 = TIDLE is used to calculate the clock low period in bus active.
     * |        |          |1 = TIDLE is used to calculate the IDLE period in bus Idle.
     ...
     * |        |          |This PECCLR bit used to enable the condition of Repeat START can clear the PEC calculation.
     ...
     * |        |          |1 = Bus clock low time-out detection Enabled (bus clock is low for more than BUSTO (I2C_BUSTOUT[7:0]) (in BIDLE=0) or high more than BUSTO (in BIDLE =1)
